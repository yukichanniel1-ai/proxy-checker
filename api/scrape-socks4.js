/**
 * scrape-socks4.js -- Standalone SOCKS4 proxy scraper + checker + Telegram sender.
 * Vercel serverless function. Trigger via GET /api/scrape-socks4 or cron.
 *
 * Everything in ONE file. Scrapes SOCKS4 proxies, checks speed (5ms-1000ms),
...
const SOURCES = [
  ...
  {
    name: "GeoNode p2",
    url: "https://proxylist.geonode.com/api/proxy-list?limit=500&page=2&sort_by=lastChecked&sort_type=desc&protocols=socks4",
    fmt: "geonode",
  },
];
    ...
    for (const r of results) {
      if (r.status === "fulfilled") {
        ...
        if (ms !== null && ms >= minMs && ms <= maxMs) {
          alive.push({ proxy, ms, type: "socks4" });
        } else {
          dead.push(proxy);
        }
      }
...
async function broadcast(alive, dead, config) {
  ...
  for (const chatId of chatIds) {
    ...
    if (alive.length > 0) {
      await telegramSendFile(token, chatId, fileContent, "socks4_proxies.txt", `${alive.length} fast SOCKS4 proxies`);
    }
  }
}
