...
async function broadcast(alive, dead, config) {
  ...
  const chatIds = allChatIds["proxy-file3"] || [];
  if (!chatIds.length || chatIds[0] === "CHAT_ID_HERE") return;
  const fileContent = alive.map(p => p.proxy).join("\n") + "\n";
  const summary = `Proxy File 3 | Fast: ${alive.length} | Dead: ${dead.length} | Fastest: ${alive.length > 0 ? alive[0].ms+"ms" : "-"}`;
  for (const chatId of chatIds) {
    await tgMsg(token, chatId, summary);
    if (alive.length > 0) {
      await tgFile(token, chatId, fileContent, "proxy_file3.txt", `${alive.length} proxies (ip:port)`);
    }
  }
}
