...
async function broadcast(alive, dead, config) {
  ...
  const summary = `<b>Proxy File 2 Update</b>\nFast (5-1000ms): ${alive.length}\n` +
  ...
  for (const chatId of chatIds) {
    ...
    if (alive.length > 0) {
      ...
      await tgFile(token, chatId, all, "proxy_file2_all.txt", `${alive.length} fast proxies`);
      for (const [type, items] of Object.entries(byType)) {
        const content = items.map(p => p.proxy).join("\n") + "\n";
        await tgFile(token, chatId, content, `proxy_file2_${type}.txt`, `${items.length} ${type.toUpperCase()}`);
