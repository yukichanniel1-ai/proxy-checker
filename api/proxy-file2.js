/**
 * proxy-file2.js -- Proxy Source Set 2
 * Sources: TheSpeedX, monosans, iplocate
 *
...
// ═══════════════════════════════════════════════════════
// PROXY SOURCES SET 2 -- TheSpeedX + monosans + iplocate
// ═══════════════════════════════════════════════════════
const SOURCES = [
...
async function broadcast(alive, dead, config) {
  ...
  const chatIds = allChatIds["proxy-file2"] || [];
  if (!chatIds.length || chatIds[0] === "CHAT_ID_HERE") return;
  const fileContent = alive.map(p => p.proxy).join("\n") + "\n";
  const summary = `Proxy File 2 | Fast: ${alive.length} | Dead: ${dead.length} | Fastest: ${alive.length > 0 ? alive[0].ms+"ms" : "-"}`;
  for (const chatId of chatIds) {
    await tgMsg(token, chatId, summary);
    if (alive.length > 0) {
      await tgFile(token, chatId, fileContent, "proxy_file2.txt", `${alive.length} proxies (ip:port)`);
    }
  }
}
...
module.exports = async function handler(req, res) {
  ...
  res.status(200).json({ file: "proxy-file2", scraped: proxies.length, alive: alive.length, dead: dead.length, elapsed_sec: elapsed, fastest: alive[0]?.ms+"ms" || null, by_type: Object.fromEntries(Object.entries(byType).map(([t,i])=>[t,i.length])), proxies: alive.map(p => ({ proxy:p.proxy, ms:p.ms, type:p.type })) });
};
